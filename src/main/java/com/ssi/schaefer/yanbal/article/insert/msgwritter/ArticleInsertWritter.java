...
public class ArticleInsertWritter {
	...
	public static void main(String folderName, String deviceType, String wamasHostIpRequested, int numberOfarticles)
			throws IOException, ClassNotFoundException, SQLException {

		String selectSkuMaxCode = "SELECT MAX(SKU_CODE) FROM PWX.SKU WHERE SKU_CODE LIKE '" + deviceType + "%'";
		List<HashMap<String, String>> mapMaxSkuCode = DatabaseQueries.executeQuery(selectSkuMaxCode,
				wamasHostIpRequested);
		...
		for (int li = 1; li <= numberOfarticles; li++) {
			...
			System.out.print("\n");
			CSVUtils.writeLine(writer, Arrays.asList(" ", "<article_insert article_id=\"" + deviceType + skuNum + "\"",
					"article_name=\"" + deviceType + skuNum + "\"",
					"art_description=\"item description\" art_has_weight=\"false\" art_weight=\"1\" art_length=\"1\" art_width=\"1\" art_height=\"1\" bundle_size_base=\"1\"	/>"));
			skuNumber++;
		}
		CSVUtils.writeLine(writer, Arrays.asList("</articles>"));
		writer.flush();
